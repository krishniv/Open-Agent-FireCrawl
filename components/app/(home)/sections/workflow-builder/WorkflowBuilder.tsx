...
function WorkflowBuilderInner({
  ...
  const template = useQuery(
    api.workflows.getTemplateByCustomId,
    currentTemplateId ? { customId: currentTemplateId } : "skip"
  );
  const updateTemplateStructure = useMutation(
    api.workflows.updateTemplateStructure
  );
  ...
  const [environment, setEnvironment] = useState<"draft" | "production">(
    "draft"
  );
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        ...
        !workflowMenuRef.current.contains(event.target as any)
      ) {
  ...
  const handleBuildFromPrompt = async (prompt: string) => {
    setIsBuilding(true);
    try {
      const response = await fetch("/api/prompt-workflow", {
        method: "POST",
        headers: {
      ...
      // Create proper node labels (React elements) for each node
      const nodesWithLabels = workflowData.nodes.map((n: any) => {
        const nodeData = n.data as any;
        const nodeName = nodeData.nodeName || n.type;
        const color = getNodeColor(n.type);
        return {
          ...n,
          data: {
            ...n.data,
            label: createNodeLabel(nodeName, color, n.type),
  ...
  const handleClearCanvas = useCallback(() => {
    setConfirmDialog({
      ...
      title: "Clear Canvas",
      description:
        "This will remove all nodes and reset to the default workflow. This action cannot be undone.",
      variant: "warning",
      onConfirm: () => {
  ...
  const createNodeLabel = (
    ...
    const nodeConfig = nodeCategory?.nodes.find(
      (n) => n.type === nodeType || n.label === label
    );
    const IconComponent = nodeConfig?.icon;

    const getTextColor = () => {
      if (nodeType === "note") return "text-white";
      if (
        nodeType === "if-else" ||
        nodeType === "while" ||
  ...
  useEffect(() => {
    if (workflow) {
      ...
      if (warnings.length > 0 && duplicateWarnings.length === 0) {
        toast.warning(
          `Found ${warnings.length} duplicate credential${warnings.length > 1 ? "s" : ""}`,
          {
  ...
  useEffect(() => {
    setNodes((nds) =>
      nds.map((node) => {
        ...
        const statusChanged =
          (node.data as any)?.executionStatus !== result?.status;
        ...
        return {
          ...
          data: {
            ...
            _executionUpdate: stoppedExecuting
              ? Date.now()
              : (node.data as any)?._executionUpdate,
          },
        };
      })
    );
    ...
    setEdges((eds) =>
      eds.map((edge) => {
        const sourceCompleted =
          nodeResults[edge.source]?.status === "completed";
  ...
  const handleDeleteNode = useCallback((nodeId: string) => {
    setConfirmDialog({
      ...
      title: "Delete Node",
      description:
        "Are you sure you want to delete this node? This will also remove all connected edges.",
      variant: "danger",
      onConfirm: () => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if ((e.ctrlKey || e.metaKey) && e.key === "c" && selectedNode && !isTyping) {
        ...
        localStorage.setItem(
          "copiedNode",
          JSON.stringify(nodes.find((n) => n.id === selectedNode.id))
        );
      }
        ...
        setNodes((nds) => {
          const updated = nds.map((node) => {
            if (node.id === nodeId) {
              ...
              if (
                data.name &&
                data.name !== (node.data as any).nodeName
              ) {
                ...
                for (const category of nodeCategories) {
                  const nodeConfig = category.nodes.find(
                    (n) => n.type === nodeType
                  );
                  if (nodeConfig) {
    ...
    >
        ...
        rightAccessory={
          <div className="flex items-center gap-0 border border-border-faint rounded-8 overflow-hidden bg-background-base">
            <button
              ...
              className={`px-12 py-6 text-label-small transition-colors ${
                environment === "draft"
      ...
      >
        {duplicateWarnings.length > 0 && (
                ...
                action: {
                  label: `${
                    duplicateWarnings.length > 1
                      ? `+${duplicateWarnings.length - 1} more`
                      : "Dismiss"
                  }`,
                  onClick: () => {
          ...
          >
            ...
            {duplicateWarnings.length} Warning
                ...
                onClick={() => {
                  if (!convexId) {
                    toast.error("Please save the workflow first", {
                      description:
                        "Make some changes and wait for auto-save to complete",
                    });
                    return;
                  }
      ...
      <div className="flex flex-1">
        ...
        >
          <div className="flex-1 relative">
            ...
            >
              ...
              <Controls className="!bg-accent-white !border-border-faint" />
              <MiniMap
          ...
          <PromptConsole
            onSubmit={handleBuildFromPrompt}
        ...
        {showTestEndpoint && workflow ? (
          ...
          />
        ) : (selectedNode?.data as any)?.nodeType?.includes("if") ||
            (selectedNode?.data as any)?.nodeType?.includes("while") ||
            (selectedNode?.data as any)?.nodeType?.includes("approval") ? (
          <LogicNodePanel
            node={selectedNode}
            nodes={nodes}
            onClose={() => setSelectedNode(null)}
          ...
          <DataNodePanel
            ...
            onUpdate={handleUpdateNodeData}
          />
        ) : (selectedNode?.data as any)?.nodeType === "extract" && selectedNode ? (
          <ExtractNodePanel
          ...
          <StartNodePanel
            ...
            onUpdate={handleUpdateNodeData}
          />
        ) : (selectedNode?.data as any)?.nodeType !== "end" &&
            (selectedNode?.data as any)?.nodeType !== "note" &&
            selectedNode ? (
          <NodePanel
            nodeData={{
              id: selectedNode.id,
          ...
          <MCPPanel
            ...
            onAddToAgent={(mcpConfig) => {
              if (targetAgentForMCP) {
                const currentTools =
                  (targetAgentForMCP.data as any).mcpTools || [];
